	...
	public void render(GraphicsContext gc) {
		...
		if (this.team == Character.ALLIES) {
			...
			gc.strokeRoundRect(x, y + 20, (DrawingUtility.MODEL_WIDTH * 0.6) * getHpRatio(), 10, 10, 10);
			 ...
			 if (isAttacking) {
				 gc.drawImage(warriorAtk.get(currentFrame), x + DrawingUtility.MODEL_WIDTH, y, -DrawingUtility.MODEL_WIDTH,
							DrawingUtility.MODEL_HEIGHT);
			}else {
				gc.drawImage(warriorWalk.get(currentFrame), x + DrawingUtility.MODEL_WIDTH, y, -DrawingUtility.MODEL_WIDTH,
						DrawingUtility.MODEL_HEIGHT);
			}
